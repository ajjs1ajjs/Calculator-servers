...
public class SizingMatrix
{
    ...
    // v3: додано MiB/s (ThroughputMiBs) та профіль IOPS у діапазонах БД, латенсі вирівняно
    // до документа D-AD-ADM-E розділ 3.2. Старі збереження (без цих полів) відкидаються.
    public const int CurrentSchemaVersion = 3;
    public int SchemaVersion { get; set; } = CurrentSchemaVersion;

    // Значення узгоджені з документом D-AD-ADM-E, розділ 3.2 «Вимоги до сервера бази даних»
    // (CPU/RAM/IOPS/латенсі) та таблицею зведення конфігурацій (MiB/s). Профіль IOPS — 50r/50w.
    public List<UserLoadRange> MsSqlRanges { get; set; } = new()
    {
        new() { MinUsers = 1, MaxUsers = 10,    Cpu = 2,  RamMin = 4,   RamRec = 8,    Iops = 200,   Latency = 10,  ThroughputMiBs = 60 },
        new() { MinUsers = 11, MaxUsers = 25,   Cpu = 4,  RamMin = 8,   RamRec = 12,   Iops = 250,   Latency = 10,  ThroughputMiBs = 120 },
        new() { MinUsers = 26, MaxUsers = 50,   Cpu = 6,  RamMin = 16,  RamRec = 24,   Iops = 300,   Latency = 8,   ThroughputMiBs = 160 },
        new() { MinUsers = 51, MaxUsers = 100,  Cpu = 8,  RamMin = 32,  RamRec = 48,   Iops = 500,   Latency = 5,   ThroughputMiBs = 240 },
        new() { MinUsers = 101, MaxUsers = 200, Cpu = 10, RamMin = 64,  RamRec = 96,   Iops = 800,   Latency = 3,   ThroughputMiBs = 500 },
        new() { MinUsers = 201, MaxUsers = 350, Cpu = 12, RamMin = 112, RamRec = 168,  Iops = 1400,  Latency = 2,   ThroughputMiBs = 600 },
        new() { MinUsers = 351, MaxUsers = 500, Cpu = 16, RamMin = 168, RamRec = 240,  Iops = 2000,  Latency = 2,   ThroughputMiBs = 700 },
        new() { MinUsers = 501, MaxUsers = 1000, Cpu = 20, RamMin = 240, RamRec = 384, Iops = 4000,  Latency = 0.6, ThroughputMiBs = 800 },
        new() { MinUsers = 1001, MaxUsers = 2000, Cpu = 22, RamMin = 384, RamRec = 576, Iops = 12000, Latency = 0.2, ThroughputMiBs = 900 },
        new() { MinUsers = 2001, MaxUsers = 3000, Cpu = 24, RamMin = 576, RamRec = 768, Iops = 24000, Latency = 0.1, ThroughputMiBs = 1000 },
        new() { MinUsers = 3001, MaxUsers = 4000, Cpu = 28, RamMin = 768, RamRec = 960, Iops = 36000, Latency = 0.1, ThroughputMiBs = 1600 },
        new() { MinUsers = 4001, MaxUsers = 5000, Cpu = 32, RamMin = 960, RamRec = 1152, Iops = 48000, Latency = 0.1, ThroughputMiBs = 2048 },
    };
    // Профіль Документообіг — варіант Standard (рекомендований як основа планування),
    // D-AD-ADM-E розділ 3.11.1 «Варіант Standard». Латенсі — за розділом 3.2; MiB/s — зі зведення.
    // Профіль IOPS — 50r/50w.
    public List<UserLoadRange> MsSqlPerformanceRanges { get; set; } = new()
    {
        new() { MinUsers = 1, MaxUsers = 10,    Cpu = 2,  RamMin = 4,   RamRec = 6,    Iops = 200,    Latency = 10,  ThroughputMiBs = 60 },
        new() { MinUsers = 11, MaxUsers = 25,   Cpu = 2,  RamMin = 6,   RamRec = 8,    Iops = 250,    Latency = 10,  ThroughputMiBs = 120 },
        new() { MinUsers = 26, MaxUsers = 50,   Cpu = 4,  RamMin = 8,   RamRec = 16,   Iops = 300,    Latency = 8,   ThroughputMiBs = 160 },
        new() { MinUsers = 51, MaxUsers = 100,  Cpu = 4,  RamMin = 16,  RamRec = 24,   Iops = 500,    Latency = 5,   ThroughputMiBs = 240 },
        new() { MinUsers = 101, MaxUsers = 200, Cpu = 4,  RamMin = 24,  RamRec = 32,   Iops = 800,    Latency = 3,   ThroughputMiBs = 500 },
        new() { MinUsers = 201, MaxUsers = 350, Cpu = 6,  RamMin = 32,  RamRec = 48,   Iops = 1400,   Latency = 2,   ThroughputMiBs = 600 },
        new() { MinUsers = 351, MaxUsers = 500, Cpu = 6,  RamMin = 48,  RamRec = 64,   Iops = 2000,   Latency = 2,   ThroughputMiBs = 700 },
        new() { MinUsers = 501, MaxUsers = 1000, Cpu = 8, RamMin = 64,  RamRec = 96,   Iops = 4000,   Latency = 0.6, ThroughputMiBs = 800 },
        new() { MinUsers = 1001, MaxUsers = 2000, Cpu = 8, RamMin = 96,  RamRec = 128, Iops = 12000,  Latency = 0.2, ThroughputMiBs = 900 },
        new() { MinUsers = 2001, MaxUsers = 3000, Cpu = 12, RamMin = 128, RamRec = 256, Iops = 36000, Latency = 0.1, ThroughputMiBs = 1000 },
        new() { MinUsers = 3001, MaxUsers = 4000, Cpu = 12, RamMin = 256, RamRec = 384, Iops = 64000, Latency = 0.1, ThroughputMiBs = 1600 },
        new() { MinUsers = 4001, MaxUsers = 5000, Cpu = 16, RamMin = 384, RamRec = 512, Iops = 128000, Latency = 0.1, ThroughputMiBs = 2048 },
    };
    ...
    // Сервери додатків Документообігу — варіант Standard (D-AD-ADM-E розділ 3.11.3 «Варіант Standard»).
    // Профіль IOPS серверів додатків — 30r/70w.
    public List<UserLoadRange> AppServerPerformanceRanges { get; set; } = new()
    {
        new() { MinUsers=1, MaxUsers=10, InstanceCount=1, Ghz=2.0, Cpu=2, Iops=250, RamMin=6, RamRec=8 },
        new() { MinUsers=11, MaxUsers=25, InstanceCount=1, Ghz=2.4, Cpu=2, Iops=250, RamMin=12, RamRec=16 },
        new() { MinUsers=26, MaxUsers=50, InstanceCount=1, Ghz=2.4, Cpu=4, Iops=500, RamMin=12, RamRec=16 },
        new() { MinUsers=51, MaxUsers=100, InstanceCount=2, Ghz=2.4, Cpu=4, Iops=500, RamMin=12, RamRec=16 },
        new() { MinUsers=101, MaxUsers=200, InstanceCount=4, Ghz=2.4, Cpu=4, Iops=500, RamMin=12, RamRec=16 },
        new() { MinUsers=201, MaxUsers=350, InstanceCount=7, Ghz=2.4, Cpu=4, Iops=500, RamMin=12, RamRec=16 },
        new() { MinUsers=351, MaxUsers=500, InstanceCount=10, Ghz=2.4, Cpu=4, Iops=500, RamMin=12, RamRec=16 },
        new() { MinUsers=501, MaxUsers=1000, InstanceCount=20, Ghz=2.4, Cpu=4, Iops=500, RamMin=12, RamRec=16 },
        new() { MinUsers=1001, MaxUsers=2000, InstanceCount=40, Ghz=2.4, Cpu=4, Iops=500, RamMin=12, RamRec=16 },
        new() { MinUsers=2001, MaxUsers=3000, InstanceCount=60, Ghz=2.4, Cpu=4, Iops=500, RamMin=12, RamRec=16 },
        new() { MinUsers=3001, MaxUsers=4000, InstanceCount=80, Ghz=2.4, Cpu=4, Iops=500, RamMin=12, RamRec=16 },
        new() { MinUsers=4001, MaxUsers=5000, InstanceCount=100, Ghz=2.4, Cpu=4, Iops=500, RamMin=12, RamRec=16 },
    };
    ...
    {
        // PostgreSQL 17+: based on HammerDB/pgbench benchmarks vs SQL Server.
        // RAM: ~33-50% від SQL Server (shared_buffers + OS cache double caching).
        // CPU: ~50-75% (ефективніший connection handling, parallel query).
        // IOPS: ~60-70% (WAL ефективніший за transaction log SQL Server).
        new() { MinUsers = 1,   MaxUsers = 25,   Cpu = 2,  RamMin = 2,  RamRec = 4,   Iops = 150,  Latency = 5 },
        new() { MinUsers = 26,  MaxUsers = 50,   Cpu = 3,  RamMin = 4,  RamRec = 8,   Iops = 200,  Latency = 3 },
        new() { MinUsers = 51,  MaxUsers = 100,  Cpu = 4,  RamMin = 8,  RamRec = 16,  Iops = 300,  Latency = 2 },
        new() { MinUsers = 101, MaxUsers = 200,  Cpu = 6,  RamMin = 16, RamRec = 32,  Iops = 500,  Latency = 1.5 },
        new() { MinUsers = 201, MaxUsers = 500,  Cpu = 8,  RamMin = 32, RamRec = 64,  Iops = 800,  Latency = 1 },
        new() { MinUsers = 501, MaxUsers = 1000, Cpu = 12, RamMin = 64, RamRec = 128, Iops = 1500, Latency = 0.5 },
        new() { MinUsers = 1001,MaxUsers = 2000, Cpu = 16, RamMin = 128,RamRec = 192, Iops = 3000, Latency = 0.3 },
        new() { MinUsers = 2001,MaxUsers = 3000, Cpu = 20, RamMin = 192,RamRec = 256, Iops = 6000, Latency = 0.2 },
        new() { MinUsers = 3001,MaxUsers = 5000, Cpu = 24, RamMin = 256,RamRec = 384, Iops = 10000,Latency = 0.1 },
    };

    public List<UserLoadRange> OracleRanges { get; set; } = new()
    {
        // Oracle 19c: based on standard benchmarks vs SQL Server.
        // RAM: ~70-100% від SQL Server (PGA+SGA with AMM can be efficient).
        // CPU: ~90-110% (parse/optimize overhead, but better execution plans).
        // IOPS: ~120-150% (redo logging + undo management overhead).
        new() { MinUsers = 1,   MaxUsers = 25,   Cpu = 2,  RamMin = 4,  RamRec = 8,   Iops = 250,  Latency = 8 },
        new() { MinUsers = 26,  MaxUsers = 50,   Cpu = 4,  RamMin = 8,  RamRec = 12,  Iops = 350,  Latency = 5 },
        new() { MinUsers = 51,  MaxUsers = 100,  Cpu = 6,  RamMin = 16, RamRec = 32,  Iops = 600,  Latency = 3 },
        new() { MinUsers = 101, MaxUsers = 200,  Cpu = 10, RamMin = 32, RamRec = 64,  Iops = 1000, Latency = 2 },
        new() { MinUsers = 201, MaxUsers = 500,  Cpu = 12, RamMin = 64, RamRec = 128, Iops = 1800, Latency = 1.5 },
        new() { MinUsers = 501, MaxUsers = 1000, Cpu = 16, RamMin = 128,RamRec = 256, Iops = 5000, Latency = 1 },
        new() { MinUsers = 1001,MaxUsers = 2000, Cpu = 22, RamMin = 256,RamRec = 384, Iops = 14000,Latency = 0.5 },
        new() { MinUsers = 2001,MaxUsers = 3000, Cpu = 24, RamMin = 384,RamRec = 512, Iops = 28000,Latency = 0.3 },
        new() { MinUsers = 3001,MaxUsers = 5000, Cpu = 28, RamMin = 512,RamRec = 768, Iops = 40000,Latency = 0.2 },
    };
    ...
    public List<ProjectModule> StandardModules { get; set; } = new()
    {
        ...
        new()
        {
            ...
            Components = new List<ModuleComponent>
            {
                ...
                new() { Name = "LMS-Videoutilities", Cpu = 4.0, RamGb = 6, Formula = ReplicaFormula.Fixed, FixedReplicas = 1, HasLocalSql = true, Notes = "Потребує GPU для перекодування відео" },
