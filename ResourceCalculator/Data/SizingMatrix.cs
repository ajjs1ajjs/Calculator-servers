...
namespace ResourceCalculator.Data;
...
public class SizingMatrix
{
    ...
    // Профіль Документообіг — значення з еталонного калькулятора (вкладка MSSQL, стовпець
    // «Документообіг»). Латенсі: 8,7,5,4,3,2,1,0.6,0.2,0.1,0.1,0.1. MiB/s — зі зведення (1.3).
    // Профіль IOPS — 50r/50w.
    public List<UserLoadRange> MsSqlPerformanceRanges { get; set; } = new()
    {
        new() { MinUsers = 1, MaxUsers = 10,    Cpu = 2,  RamMin = 4,   RamRec = 8,    Iops = 200,    Latency = 8,   ThroughputMiBs = 60 },
        new() { MinUsers = 11, MaxUsers = 25,   Cpu = 4,  RamMin = 8,   RamRec = 12,   Iops = 250,    Latency = 7,   ThroughputMiBs = 120 },
        new() { MinUsers = 26, MaxUsers = 50,   Cpu = 4,  RamMin = 16,  RamRec = 24,   Iops = 300,    Latency = 5,   ThroughputMiBs = 160 },
        new() { MinUsers = 51, MaxUsers = 100,  Cpu = 6,  RamMin = 32,  RamRec = 48,   Iops = 500,    Latency = 4,   ThroughputMiBs = 240 },
        new() { MinUsers = 101, MaxUsers = 200, Cpu = 6,  RamMin = 48,  RamRec = 64,   Iops = 800,    Latency = 3,   ThroughputMiBs = 500 },
        new() { MinUsers = 201, MaxUsers = 350, Cpu = 8,  RamMin = 64,  RamRec = 96,   Iops = 1400,   Latency = 2,   ThroughputMiBs = 600 },
        new() { MinUsers = 351, MaxUsers = 500, Cpu = 8,  RamMin = 96,  RamRec = 128,  Iops = 2000,   Latency = 1,   ThroughputMiBs = 700 },
        new() { MinUsers = 501, MaxUsers = 1000, Cpu = 12, RamMin = 128, RamRec = 192, Iops = 4000,   Latency = 0.6, ThroughputMiBs = 800 },
        new() { MinUsers = 1001, MaxUsers = 2000, Cpu = 12, RamMin = 192, RamRec = 378, Iops = 12000, Latency = 0.2, ThroughputMiBs = 900 },
        new() { MinUsers = 2001, MaxUsers = 3000, Cpu = 16, RamMin = 378, RamRec = 512, Iops = 36000, Latency = 0.1, ThroughputMiBs = 1000 },
    ...
    // Сервери додатків Документообігу — значення з еталонного калькулятора (вкладка Windows →
    // AppServers «Документообіг»): IOPS 1200 (30r/70w), RAM 24/32, к-сть серверів 1…67.
