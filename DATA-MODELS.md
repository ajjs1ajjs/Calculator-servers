...
<!-- AUTO:stamp -->Verified: 2026-09-17, commit `1571551` (scripts/Update-Docs.ps1)<!-- /AUTO -->
...
| Властивість | Тип | Опис |
|---|---|---|
| `Slot` | `NodeSlot` | **Роль вузла — явна.** `None/K8sSql/K8sMaster/K8sWorker/WindowsSql/WindowsApp/WindowsWeb/ReportingServer/HaProxy`. Задається кодом (дефолти `SizingMatrix`), не користувачем: саме за нею `MatrixManager` повертає відредагований рядок гріда у правильний слот. Раніше маппінг ішов через `Name.Contains(...)` і ламався на назвах, що перетинаються («Веб сервери (IIS)» містить «сервер» → падав у слот сервера додатків). `MatrixValidator` відкидає файл із чужим слотом |
| `Name` | `string` | Назва вузла (напр. `"K8s SQL"`, `"K8s Master"`). Вільний текст, на логіку не впливає |
...
**SchemaVersion**: `11` (відкидає застарі матриці). Версія 11 додала `InfrastructureNode.Slot` — збережений `matrix.json` версії 10 відкидається, локальні правки матриці треба внести заново.
